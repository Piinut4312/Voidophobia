...
public class ReinforcedPistonBlockEntity extends BlockEntity {
    ...
    public ReinforcedPistonBlockEntity(BlockPos pos, BlockState state) {
        super(ModBlockEntities.REINFORCED_PISTON, pos, state);
    }

    public ReinforcedPistonBlockEntity(BlockPos pos, BlockState state, BlockState pushedBlock, Direction facing, boolean extending, boolean source) {
        this(pos, state);
    ...
    private BlockState getHeadBlockState() {
        if (!this.isExtending() && this.isSource() && this.pushedBlock.getBlock() instanceof ReinforcedPistonBlock) {
            return ModBlocks.REINFORCED_PISTON_HEAD.getDefaultState().with(ReinforcedPistonHeadBlock.SHORT, this.progress > 0.25f).with(ReinforcedPistonHeadBlock.TYPE, this.pushedBlock.isOf(ModBlocks.REINFORCED_PISTON) ? PistonType.DEFAULT : PistonType.STICKY).with(ReinforcedPistonHeadBlock.FACING, this.pushedBlock.get(ReinforcedPistonBlock.FACING));
        }
        return this.pushedBlock;
    }

    private static void pushEntities(World world, BlockPos pos, float f, ReinforcedPistonBlockEntity blockEntity) {
        ...
        for (Entity entity : list) {
            ...
            while (!(!iterator.hasNext() || (box3 = Boxes.stretch(offsetHeadBox(pos, iterator.next(), blockEntity), direction, d)).intersects(box4 = entity.getBoundingBox()) && (i = Math.max(i, getIntersectionSize(box3, direction, box4))) >= d)) {
            }
            if (i <= 0.0) continue;
            i = Math.min(i, d) + 0.01;
    ...
    private static void moveEntitiesInHoneyBlock(World world, BlockPos pos, float f, ReinforcedPistonBlockEntity blockEntity) {
        ...
        for (Entity entity2 : list) {
            moveEntity(direction, entity2, e, direction);
        }
    }
    ...
    private static Box offsetHeadBox(BlockPos pos, Box box, ReinforcedPistonBlockEntity blockEntity) {
    ...
    private static void push(BlockPos pos, Entity entity, Direction direction, double amount) {
        ...
        if (box.intersects(box2 = VoxelShapes.fullCube().getBoundingBox().offset(pos)) && Math.abs((d = getIntersectionSize(box2, direction2 = direction.getOpposite(), box) + 0.01) - (getIntersectionSize(box2, direction2, box.intersection(box2)) + 0.01)) < 0.01) {
    ...
    public void finish() {
        if (this.world != null && (this.lastProgress < 1.0f || this.world.isClient)) {
            ...
            this.markRemoved();
            if (this.world.getBlockState(this.pos).isOf(ModBlocks.MOVING_REINFORCED_PISTON)) {
    ...
    public static void tick(World world, BlockPos pos, BlockState state, ReinforcedPistonBlockEntity blockEntity) {
        ...
        if (blockEntity.lastProgress >= 1.0f) {
            ...
            if (world.getBlockState(pos).isOf(ModBlocks.MOVING_REINFORCED_PISTON)) {
    ...
    public VoxelShape getCollisionShape(BlockView world, BlockPos pos) {
        VoxelShape voxelShape = !this.extending && this.source && this.pushedBlock.getBlock() instanceof ReinforcedPistonBlock ? this.pushedBlock.with(ReinforcedPistonBlock.EXTENDED, true).getCollisionShape(world, pos) : VoxelShapes.empty();
        Direction direction = field_12205.get();
        if ((double)this.progress < 1.0 && direction == this.getMovementDirection()) {
            return voxelShape;
        }
        BlockState blockState = this.isSource() ? ModBlocks.REINFORCED_PISTON_HEAD.getDefaultState().with(ReinforcedPistonHeadBlock.FACING, this.facing).with(ReinforcedPistonHeadBlock.SHORT, this.extending != 1.0f - this.progress < 0.25f) : this.pushedBlock;
