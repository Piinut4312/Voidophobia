...
public class ReinforcedPistonHandler {
    public static final int MAX_MOVABLE_BLOCKS = 18;
    ...
    public ReinforcedPistonHandler(World world, BlockPos pos, Direction dir, boolean retracted) {
        this.world = world;
        this.posFrom = pos;
    ...
    public boolean calculatePush() {
        ...
        if (!ReinforcedPistonBlock.isMovable(blockState, this.world, this.posTo, this.motionDirection, false, this.pistonDirection)) {
        ...
        for (int i = 0; i < this.movedBlocks.size(); ++i) {
            ...
            if (!isBlockSticky(this.world.getBlockState(blockPos)) || this.tryMoveAdjacentBlock(blockPos)) continue;
            return false;
        }
        return true;
    }
    ...
    private boolean tryMove(BlockPos pos, Direction dir) {
        ...
        if (!ReinforcedPistonBlock.isMovable(blockState, this.world, pos, this.motionDirection, false, dir)) {
            return true;
        }
        ...
        while (ReinforcedPistonHandler.isBlockSticky(blockState)) {
            BlockPos blockPos = pos.offset(this.motionDirection.getOpposite(), i);
            BlockState blockState2 = blockState;
            blockState = this.world.getBlockState(blockPos);
            if (blockState.isAir() || !isAdjacentBlockStuck(blockState2, blockState) || !ReinforcedPistonBlock.isMovable(blockState, this.world, blockPos, this.motionDirection, false, this.motionDirection.getOpposite()) || blockPos.equals(this.posFrom)) break;
        ...
        while (true) {
            ...
            if (!ReinforcedPistonBlock.isMovable(blockState, this.world, blockPos2, this.motionDirection, true, this.motionDirection) || blockPos2.equals(this.posFrom)) {
                return false;
            }
